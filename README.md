...
设置以下变量可将多个账号的 Key 合并到同一个代理接口。请求按轮询（Round-Robin）分配，当某个 Key 触发 429 限速时自动切换下一个 Key 重试：

| 变量名 | 格式 | 说明 |
|--------|------|------|
| `OPENAI_API_KEYS` | `sk-key1,sk-key2,sk-key3` | 多个 OpenAI Key 逗号分隔，优先于 `AI_INTEGRATIONS_OPENAI_API_KEY` |
| `OPENAI_BASE_URL` | `https://api.openai.com/v1` | 所有 OpenAI Key 共用同一 Base URL |
| `ANTHROPIC_API_KEYS` | `sk-ant-key1,sk-ant-key2` | 多个 Anthropic Key 逗号分隔，优先于 `AI_INTEGRATIONS_ANTHROPIC_API_KEY` |
| `ANTHROPIC_BASE_URL` | `https://api.anthropic.com` | 所有 Anthropic Key 共用同一 Base URL |
| `PROXY_API_KEY` | 任意字符串 | 客户端访问本代理的鉴权 Key |
